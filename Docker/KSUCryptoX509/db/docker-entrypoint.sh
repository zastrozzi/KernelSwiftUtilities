...
pg_setup_hba_conf() {
	...
	: "${POSTGRES_HOST_AUTH_METHOD:=$auth}"
	{
		...
		printf 'hostssl all all all cert\n'
	} >> "$PGDATA/pg_hba.conf"
}

# add ssl config to postgresql.conf
pg_setup_ssl_conf() {
	cp -r "$CERTDIR" "$PGDATA"
	chown -R postgres:postgres "$PGDATA"/certs/postgresdb.crt && chmod 0600 "$PGDATA"/certs/postgresdb.crt
	chown -R postgres:postgres "$PGDATA"/certs/postgresdb.key && chmod 0600 "$PGDATA"/certs/postgresdb.key
	{
		printf '\n'
		printf '# CUSTOM SSL SETTINGS\n'
		printf 'ssl = on\n'
		printf "ssl_ca_file = 'certs/myCA.crt'\n"
		printf "ssl_cert_file = 'certs/postgresdb.crt'\n"
		printf "ssl_key_file = 'certs/postgresdb.key'\n"
