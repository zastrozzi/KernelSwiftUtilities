...
extension KernelDynamicQuery.Fluent.Migrations {
    public struct EnumFilter_Create_v1_0: AsyncMigration {
        public func prepare(on database: any Database) async throws {
            try await database.schema(SchemaName.enumFilter)
                .id()
                .timestamps()
                .group("col", columnIdentifiers)
                .field("field_array", .bool, .required)
                .field("enum_name", .string, .required)
        ...
        public func revert(on database: any Database) async throws {
            try await database.schema(SchemaName.enumFilter).delete()
        }
    }
}
