...
extension KernelLocation.Fluent.Migrations {
    public struct UKOrdnanceSurveyBoundaryLineHistoricEuropeanRegion_Create_v1_0: AsyncMigration {
        public func prepare(on database: any Database) async throws {
            try await database.schema(SchemaName.ukOrdnanceSurveyBoundaryLineHistoricEuropeanRegion)
        ...
        public func revert(on database: any Database) async throws {
            try await database.schema(SchemaName.ukOrdnanceSurveyBoundaryLineHistoricEuropeanRegion).delete()
        }
    }
    
    public struct UKOrdnanceSurveyBoundaryLineHistoricEuropeanRegion_AddGeometryConversionTrigger: AsyncMigration {
        let tableName: String = SchemaName.ukOrdnanceSurveyBoundaryLineHistoricEuropeanRegion.namespaceResolvedTable
        let functionName: String = SchemaName.ukOrdnanceSurveyBoundaryLineHistoricEuropeanRegion.namespaceResolvedTable + "_gc()"
        let triggerName: String = SchemaName.ukOrdnanceSurveyBoundaryLineHistoricEuropeanRegion.table + "_gc"
