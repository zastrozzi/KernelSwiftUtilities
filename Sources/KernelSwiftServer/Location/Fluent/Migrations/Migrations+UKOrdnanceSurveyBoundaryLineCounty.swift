...
extension KernelLocation.Fluent.Migrations {
    public struct UKOrdnanceSurveyBoundaryLineCounty_Create_v1_0: AsyncMigration {
        public func prepare(on database: any Database) async throws {
            try await database.schema(SchemaName.ukOrdnanceSurveyBoundaryLineCounty)
        ...
        public func revert(on database: any Database) async throws {
            try await database.schema(SchemaName.ukOrdnanceSurveyBoundaryLineCounty).delete()
        }
    }
    
    public struct UKOrdnanceSurveyBoundaryLineCounty_AddGeometryConversionTrigger: AsyncMigration {
        let tableName: String = SchemaName.ukOrdnanceSurveyBoundaryLineCounty.namespaceResolvedTable
        let functionName: String = SchemaName.ukOrdnanceSurveyBoundaryLineCounty.namespaceResolvedTable + "_gc()"
        let triggerName: String = SchemaName.ukOrdnanceSurveyBoundaryLineCounty.table + "_gc"
