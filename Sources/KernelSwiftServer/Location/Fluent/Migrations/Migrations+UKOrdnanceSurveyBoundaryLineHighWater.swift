...
extension KernelLocation.Fluent.Migrations {
    public struct UKOrdnanceSurveyBoundaryLineHighWater_Create_v1_0: AsyncMigration {
        public func prepare(on database: any Database) async throws {
            try await database.schema(SchemaName.ukOrdnanceSurveyBoundaryLineHighWater)
                .field(.id, .uuid, .required, .identifier(auto: false), .sql(.default(SQLFunction("gen_random_uuid"))))
                .timestampsWithDefaults()
                .field("geometry", .geometricMultiLineString2D(.osgb36BritishNationalGrid), .required)
                .field("geometry_wgs84", .geometricMultiLineString2D(.wgs84), .required)
                .field("feature_code", .string, .required)
                .field("feature_description", .string, .required)
                .field("file_name", .string, .required)
                .field("feature_serial_number", .int32, .required)
                .field("global_link_id", .int32, .required)
                .unique(on: "feature_serial_number")
                .create()
        }
        
        public func revert(on database: any Database) async throws {
            try await database.schema(SchemaName.ukOrdnanceSurveyBoundaryLineHighWater).delete()
        }
    }
    
    public struct UKOrdnanceSurveyBoundaryLineHighWater_AddGeometryConversionTrigger: AsyncMigration {
        let tableName: String = SchemaName.ukOrdnanceSurveyBoundaryLineHighWater.namespaceResolvedTable
        let functionName: String = SchemaName.ukOrdnanceSurveyBoundaryLineHighWater.namespaceResolvedTable + "_gc()"
        let triggerName: String = SchemaName.ukOrdnanceSurveyBoundaryLineHighWater.table + "_gc"
