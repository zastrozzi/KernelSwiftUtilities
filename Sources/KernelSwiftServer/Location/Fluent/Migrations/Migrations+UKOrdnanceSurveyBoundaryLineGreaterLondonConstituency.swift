...
extension KernelLocation.Fluent.Migrations {
    public struct UKOrdnanceSurveyBoundaryLineGreaterLondonConstituency_Create_v1_0: AsyncMigration {
        public func prepare(on database: any Database) async throws {
            try await database.schema(SchemaName.ukOrdnanceSurveyBoundaryLineGreaterLondonConstituency)
        ...
        public func revert(on database: any Database) async throws {
            try await database.schema(SchemaName.ukOrdnanceSurveyBoundaryLineGreaterLondonConstituency).delete()
        }
    }
    
    public struct UKOrdnanceSurveyBoundaryLineGreaterLondonConstituency_AddGeometryConversionTrigger: AsyncMigration {
        let tableName: String = SchemaName.ukOrdnanceSurveyBoundaryLineGreaterLondonConstituency.namespaceResolvedTable
        let functionName: String = SchemaName.ukOrdnanceSurveyBoundaryLineGreaterLondonConstituency.namespaceResolvedTable + "_gc()"
        let triggerName: String = SchemaName.ukOrdnanceSurveyBoundaryLineGreaterLondonConstituency.table + "_gc"
