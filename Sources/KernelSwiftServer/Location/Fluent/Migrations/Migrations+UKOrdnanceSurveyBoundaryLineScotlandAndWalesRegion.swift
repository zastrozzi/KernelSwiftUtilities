...
extension KernelLocation.Fluent.Migrations {
    public struct UKOrdnanceSurveyBoundaryLineScotlandAndWalesRegion_Create_v1_0: AsyncMigration {
        public func prepare(on database: any Database) async throws {
            try await database.schema(SchemaName.ukOrdnanceSurveyBoundaryLineScotlandAndWalesRegion)
        ...
        public func revert(on database: any Database) async throws {
            try await database.schema(SchemaName.ukOrdnanceSurveyBoundaryLineScotlandAndWalesRegion).delete()
        }
    }
    
    public struct UKOrdnanceSurveyBoundaryLineScotlandAndWalesRegion_AddGeometryConversionTrigger: AsyncMigration {
        let tableName: String = SchemaName.ukOrdnanceSurveyBoundaryLineScotlandAndWalesRegion.namespaceResolvedTable
        let functionName: String = SchemaName.ukOrdnanceSurveyBoundaryLineScotlandAndWalesRegion.namespaceResolvedTable + "_gc()"
        let triggerName: String = SchemaName.ukOrdnanceSurveyBoundaryLineScotlandAndWalesRegion.table + "_gc"
