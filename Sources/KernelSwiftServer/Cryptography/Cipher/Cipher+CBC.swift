...
extension KernelCryptography.Cipher {
    public struct CBC: CipherStrategy {
        public static let blockMode: KernelCryptography.Cipher.BlockMode = .CBC
...
//                print("Encrypt")
                ...
                for i in .zero..<b.count { b[i] ^= xor[i] }
                ...
                b[0 ..< AES.val.blockSize] = input[index ..< index + AES.val.blockSize]
                let w = b
                b = AES.decrypt(b, aes: &aes)
                for i in .zero..<b.count { input[index + i] = b[i] ^ xor[i] }
                xor = w
                index += AES.val.blockSize
                remaining -= AES.val.blockSize
                if remaining <= .zero {
                    let padCount: Int = .init(input[index - 1])
                    if padCount > AES.val.blockSize {
                        print(padCount, remaining)
                        throw KernelCryptography.TypedError(.invalidAESBufferLength)
                    }
