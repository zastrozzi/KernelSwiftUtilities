...
extension KernelXML.XMLStackParser: XMLParserDelegate {
                ...
                attributes attributeDict: [String: String] = [:])
    {
        let attributes = attributeDict.map { key, value in
            KernelXML.XMLCoderElement.Attribute(key: key, value: value)
        }
        let element = KernelXML.XMLCoderElement(key: elementName, attributes: attributes)
    ...
    func elementWithFilteredElements(element: KernelXML.XMLCoderElement) -> KernelXML.XMLCoderElement {
        var hasWhitespaceElements = false
        var hasNonWhitespaceElements = false
        var filteredElements: [KernelXML.XMLCoderElement] = []
        ...
        if hasWhitespaceElements && hasNonWhitespaceElements {
            return KernelXML.XMLCoderElement(key: element.key, elements: filteredElements, attributes: element.attributes)
        }
        return element
    }
