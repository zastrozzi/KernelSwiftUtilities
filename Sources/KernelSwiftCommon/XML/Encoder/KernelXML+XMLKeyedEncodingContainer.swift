...
extension KernelXML {
    struct XMLKeyedEncodingContainer<K: CodingKey>: KeyedEncodingContainerProtocol {
        ...
        private mutating func encode<T: Encodable>(
            _ value: T,
            forKey key: Key,
            encode: (SynchronousXMLEncoder, T) throws -> KernelXML.Boxable
        ) throws {
            defer {
            ...
            let oldSelf = self
            let attributeEncoder: (T, Key, KernelXML.Boxable) throws -> () = { value, key, box in
                guard let attribute = box as? KernelXML.SimpleBoxable else {
            ...
            let elementEncoder: (T, Key, KernelXML.Boxable) throws -> () = { _, key, box in
                oldSelf.container.withShared { container in
            ...
            default:
                switch value {
                case is KernelXML.ElementRepresentable:
                    encodeElement(forKey: key, box: box)
                case is KernelXML.Attributable:
                    try encodeAttribute(value, forKey: key, box: box)
                case is KernelXML.ElementAndAttributeRepresentable:
                    try encodeAttribute(value, forKey: key, box: box)
                    encodeElement(forKey: key, box: box)
                default:
                    encodeElement(forKey: key, box: box)
        ...
        mutating func nestedChoiceContainer<NestedKey>(
            ...
            let container = KernelXML.XMLChoiceEncodingContainer<NestedKey>(
                referencing: encoder,
                codingPath: codingPath,
        ...
        public mutating func nestedUnkeyedContainer(
            ...
            codingPath.append(key)
            defer { self.codingPath.removeLast() }
            return KernelXML.XMLUnkeyedEncodingContainer(
                referencing: encoder,
                codingPath: codingPath,
        ...
        public mutating func superEncoder() -> Encoder {
            return KernelXML.XMLReferencingEncoder(
                referencing: encoder,
                key: XMLKey.super,
