...
extension KernelNetworking {
    internal enum RuntimeError: Error, CustomStringConvertible, LocalizedError, PrettyStringConvertible {
        ...
        case unsupportedParameterStyle(name: String, location: ParameterLocation, style: Configuration.ParameterStyle, explode: Bool)
        ...
        var prettyDescription: String {
            ...
            case .invalidServerVariableValue(name: let name, value: let value, allowedValues: let allowedValues):
                return "Invalid server variable named: '\(name)', which has the value: '\(value)', but the only allowed values are: \(allowedValues.map { "'\($0)'" }.joined(separator: ", "))"
            ...
            case .unsupportedParameterStyle(name: let name, location: let location, style: let style, explode: let explode):
                return "Unsupported parameter style, parameter name: '\(name)', kind: \(location), style: \(style), explode: \(explode)"
...
extension KernelNetworking {
    public static func throwUnexpectedResponseStatus(
    ...
    public static func throwUnexpectedResponseBody(
...
extension KernelNetworking.RuntimeError: HTTPResponseConvertible {
