...
extension KernelCasePath {
    public init(_ embed: @escaping (Value) -> Root) {
        func openFunc<Wrapped>(_: Wrapped.Type) -> (Root) -> Value? {
            optionalPromotedExtractHelper(unsafeBitCast(embed, to: ((Value) -> Wrapped?).self)) as! (Root) -> Value?
        }
        
        let extract = ((_Witness<Root>.self as? _AnyOptional.Type)?.wrappedType).map { _openExistential($0, do: openFunc) } ?? extractHelper(embed)
        self.init(embed: embed, extract: extract)
    }
}

extension KernelCasePath where Value == Void {
    public init(_ root: Root) {
        func openFunc<Wrapped>(_: Wrapped.Type) -> (Root) -> Void? {
            optionalPromotedExtractVoidHelper(unsafeBitCast(root, to: Wrapped?.self)) as! (Root) -> Void?
        }
        
        let extract = ((_Witness<Root>.self as? _AnyOptional.Type)?.wrappedType).map { _openExistential($0, do: openFunc) } ?? extractVoidHelper(root)
...
func extractHelper<Root, Value>(_ embed: @escaping (Value) -> Root) -> (Root) -> Value? {
    guard let metadata = EnumMetadata(Root.self), metadata.typeDescriptor.fieldDescriptor != nil else {
        assertionFailure("embed parameter must be a valid enum case initialiser")
        return { _ in nil }
    }
    ...
    return { root in
        ...
        guard let value = rootStrategy.extract(from: root, tag: rootTag) else { return nil }
...
func optionalPromotedExtractHelper<Root, Value>(_ embed: @escaping (Value) -> Root?) -> (Root?) -> Value? {
...
func extractVoidHelper<Root>(_ root: Root) -> (Root) -> Void? {
    guard let metadata = EnumMetadata(Root.self), metadata.typeDescriptor.fieldDescriptor != nil else {
...
func optionalPromotedExtractVoidHelper<Root>(_ root: Root?) -> (Root?) -> Void? {
    guard let root = root, let metadata = EnumMetadata(Root.self), metadata.typeDescriptor.fieldDescriptor != nil else {
        assertionFailure("value must be a valid enum case")
        return { _ in nil }
    }
...
extension Strategy {
    init(tag: UInt32, assumedAssociatedValueType: Any.Type? = nil) {
        ...
        let associatedValueType = assumedAssociatedValueType ?? metadata.associatedValueType(forTag: tag)
        ...
        if associatedValueType == Value.self {
            self = .init(nonExistentialTag: tag)
        } else if shouldWorkAroundSR12044, MemoryLayout<Value>.size == 0, !isUninhabitedEnum {
            self = .void
        } else if let associatedValueMetadata = TupleMetadata(associatedValueType), associatedValueMetadata.elementCount == 1 {
            self.init(tag: tag, assumedAssociatedValueType: associatedValueMetadata.element(at: 0).type)
        } else if let associateValueMetadata = TupleMetadata(associatedValueType), let valueMetadata = TupleMetadata(Value.self), valueMetadata.labels == nil {
            guard associateValueMetadata.hasSameLayout(as: valueMetadata) else {
                self = .unimplemented
                return
            }
            self.init(tag: tag, assumedAssociatedValueType: Value.self)
        } else if let associatedValueMetadata = ExistentialMetadata(associatedValueType) {
            if associatedValueType == Error.self || associatedValueMetadata.isClassConstrained {
        ...
        } else if associatedValueType == Value?.self {
            let wrappedStrategy = Strategy<Enum, Value?>(tag: tag, assumedAssociatedValueType: associatedValueType)
    ...
    func extract(from root: Enum, tag: UInt32) -> Value? {
        ...
        case .indirect:
            return self.withProjectedPayload(of: root, tag: tag) { $0.load(as: UnsafeRawPointer.self).advanced(by: 2 * Sizes.pointerSize).load(as: Value.self) }
    ...
    private func withProjectedPayload<Answer>(of root: Enum, tag: UInt32, do body: (UnsafeRawPointer) -> Answer) -> Answer {
        var root = root
...
extension Metadata {
    var valueWitnessTable: ValueWitnessTable {
        ValueWitnessTable(ptr: self.ptr.load(fromByteOffset: -Sizes.pointerSize, as: UnsafeRawPointer.self))
    }
...
private struct EnumMetadata: Metadata {
    ...
    var genericArguments: GenericArgumentVector? {
        ...
        return .init(ptr: self.ptr.advanced(by: 2 * Sizes.pointerSize))
    }
    
    var typeDescriptor: EnumTypeDescriptor {
        EnumTypeDescriptor(ptr: self.ptr.load(fromByteOffset: Sizes.pointerSize, as: UnsafeRawPointer.self))
    }
...
extension EnumMetadata {
    func associatedValueType(forTag tag: UInt32) -> Any.Type {
        guard
            ...
            let type = swift_getTypeByMangledNameInContext(typeName.ptr, typeName.length, genericContext: self.typeDescriptor.ptr, genericArguments: self.genericArguments?.ptr)
        else {
            return Void.self
        }
...
private struct EnumTypeDescriptor: Equatable {
    ...
    var fieldDescriptor: FieldDescriptor? {
        self.ptr.advanced(by: 4 * 4).loadRelativePointer().map(FieldDescriptor.init)
    }
...
private struct TupleMetadata: Metadata {
    ...
    var elementCount: UInt {
        self.ptr.advanced(by: Sizes.pointerSize).load(as: UInt.self)
    }
    
    var labels: UnsafePointer<UInt8>? {
        self.ptr.advanced(by: Sizes.pointerSize).advanced(by: Sizes.pointerSize).load(as: UnsafePointer<UInt8>?.self)
    }
    
    func element(at i: Int) -> Element {
        Element(ptr: self.ptr.advanced(by: Sizes.pointerSize).advanced(by: Sizes.pointerSize).advanced(by: Sizes.pointerSize).advanced(by: i * 2 * Sizes.pointerSize))
    }
}
...
extension TupleMetadata {
    struct Element: Equatable {
        ...
        var offset: UInt32 { self.ptr.load(fromByteOffset: Sizes.pointerSize, as: UInt32.self) }
...
private struct ExistentialMetadata: Metadata {
    ...
    var isClassConstrained: Bool {
        self.ptr.advanced(by: Sizes.pointerSize).load(as: UInt32.self).and(0x8000_0000, equals: .zero)
    }
}
...
private struct FieldDescriptor {
    ...
    func field(atIndex i: UInt32) -> FieldRecord {
        FieldRecord(ptr: self.ptr.advanced(by: 2 * 4 + 2 * 2 + 4).advanced(by: Int(i) * recordSize))
    }
}
...
private struct MangledTypeName {
    ...
    var length: UInt {
        ...
        while true {
            ...
            case 0x18...0x1f:
                ptr = ptr.advanced(by: 1 + Sizes.pointerSize)
            default:
...
private struct ValueWitnessTable {
    ...
    var getEnumTag: @convention(c) (_ value: UnsafeRawPointer, _ metadata: UnsafeRawPointer) -> UInt32 {
        self.ptr.advanced(by: 10 * Sizes.pointerSize + 2 * 4).loadInferredType()
    }
    
    var destructiveProjectEnumData: @convention(c) (_ value: UnsafeMutableRawPointer, _ metadata: UnsafeRawPointer) -> Void {
        self.ptr.advanced(by: 11 * Sizes.pointerSize + 2 * 4).loadInferredType()
    }
    
    var destructiveInjectEnumData: @convention(c) (_ value: UnsafeMutableRawPointer, _ tag: UInt32, _ metadata: UnsafeRawPointer) -> Void {
        self.ptr.advanced(by: 12 * Sizes.pointerSize + 2 * 4).loadInferredType()
    }
}
...
extension GenericArgumentVector {
    func type(atIndex i: Int) -> Any.Type {
        return ptr.load(fromByteOffset: i * Sizes.pointerSize, as: Any.Type.self)
    }
}
