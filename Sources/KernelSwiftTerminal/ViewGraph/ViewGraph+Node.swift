...
extension KernelSwiftTerminal.ViewGraph {
    public final class Node {
        ...
        var environment: ((inout KernelSwiftTerminal.Model.EnvironmentValues) -> Void)?
        #if os(macOS)
        var subscriptions: [String: AnyCancellable] = [:]
        #endif
        
        var control: KernelSwiftTerminal.Application.Control?
        ...
        func build(rootId: UUID? = nil) {
            if !built {
                self.view.buildNode(self)
                if let rootId {
                    self.control?.id = rootId
                }
                built = true
        ...
        func control(at offset: Int) -> KernelSwiftTerminal.Application.Control {
            build()
