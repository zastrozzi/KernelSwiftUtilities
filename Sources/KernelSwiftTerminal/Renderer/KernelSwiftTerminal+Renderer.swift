...
extension KernelDI.Injector {
    public var renderer: KernelSwiftTerminal.Renderer {
        get { self[KernelSwiftTerminal.Renderer.Token.self] }
        set { self[KernelSwiftTerminal.Renderer.Token.self] = newValue}
    }
}

extension KernelSwiftTerminal {
    public class Renderer: KernelDI.Injectable {
        private var _layer: Layer? = nil
        
        var layer: Layer {
            get { precondition(_layer != nil); return _layer! }
            set { _layer = newValue }
        }
        ...
        private var cache: [[Layout.Cell?]] = []
        
        /// The current cursor position, which might need to be updated before
        /// printing.
        private var currentPosition: Layout.Position = .zero
        
        private var currentForegroundColor: Style.Color? = nil
        private var currentBackgroundColor: Style.Color? = nil
        
        private var currentAttributes = Style.CellAttributes()
        
        required public init() {}
        
        public func initialise(_ layer: Layer) {
            self.layer = layer
            setCache()
            setup()
        }
        ...
        func draw(rect: Layout.Rect? = nil) {
            if rect == nil { layer.invalidated = nil }
            let rect = rect ?? .init(position: .zero, size: layer.frame.size)
            ...
            for line in rect.minLine ... rect.maxLine {
                for column in rect.minColumn ... rect.maxColumn {
                    let position: Layout.Position = .init(column: column, line: line)
                    if let cell = layer.cell(at: position) {
                        drawPixel(cell, at: .init(column: column, line: line))
                    }
                }
            }
        }
        ...
        private func drawPixel(_ cell: Layout.Cell, at position: Layout.Position) {
        ...
        private func updateAttributes(_ attributes: Style.CellAttributes) {
