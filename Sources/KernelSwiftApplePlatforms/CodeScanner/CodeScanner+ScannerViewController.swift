    ...
    public final class ScannerViewController: UIViewController, UINavigationControllerDelegate {
        ...
        @objc func updateOrientation() {
            guard let orientation = view.window?.windowScene?.interfaceOrientation else {
                logger.debug("NO ORIENTATION")
                return
            }
            guard let connection = captureSession?.connections.last else {
...
extension KernelAppUtils.CodeScanner.ScannerViewController: @preconcurrency AVCaptureMetadataOutputObjectsDelegate {
    public func metadataOutput(_ output: AVCaptureMetadataOutput, didOutput metadataObjects: [AVMetadataObject], from connection: AVCaptureConnection) {
        ...
        handler = { [weak self] image in
            ...
            let result = KernelAppUtils.CodeScanner.ScanResult(string: stringValue, type: readableObject.type, image: image, corners: readableObject.corners)
...
extension KernelAppUtils.CodeScanner.ScannerViewController: UIImagePickerControllerDelegate {
    public func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey: Any]) {
        ...
        for feature in features.compactMap({ $0 as? CIQRCodeFeature }) {
            ...
            let result = KernelAppUtils.CodeScanner.ScanResult(string: qrCodeLink, type: .qr, image: qrcodeImg, corners: corners)
            found(result)
        }
    }
...
extension KernelAppUtils.CodeScanner.ScannerViewController: UIAdaptivePresentationControllerDelegate {
    public func presentationControllerDidDismiss(_ presentationController: UIPresentationController) {
        isGalleryShowing = false
    }
}

extension KernelAppUtils.CodeScanner.ScannerViewController: @preconcurrency AVCapturePhotoCaptureDelegate {
